...
import type { DrinkEditor } from "#/app/modules/drinks/drinks";
...
export function DrinkForm({
  ...
  const [slugValue, setSlugValue] = useState(editor?.initialValues.slug ?? "");
  const [imageRequired, setImageRequired] = useState(false);
  const [statusValue, setStatusValue] = useState(editor?.initialValues.status ?? "published");

  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    event.preventDefault();
    const form = event.currentTarget;
    const croppedBlob = await imageCropRef.current?.getCroppedImage();
    const hasExistingImage = Boolean(editor?.imageUrl);
  ...
  return (
    ...
    >
      ...
      <div>
        ...
        <div className="mt-2">
          <ImageCrop ref={imageCropRef} existingImageUrl={editor?.imageUrl} />
        </div>
